...
- **深色模式：本项目统一浅色主题，禁止再出现半成品深色模式。**
  2026-09-12 真机踩坑：`app.wxss` 里一段只改 `page` 背景的 `prefers-color-scheme: dark`
  让 **18 个「没自带背景色」的页面**（我的/成就/关卡/拼词/结算/错题本…）在深色模式手机上整片发黑，
  而开发者工具默认浅色、本地复现不出来。护栏见 `e2e/check-wxss.js`（出现该媒体查询直接判红）。
  要做深色模式必须逐页完整配色 + 补深色截图回归。
