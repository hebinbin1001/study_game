...
### 玩法页布局体检：能一屏显示的就别滚动、别留白

- 起因：用户反馈「24 点布局没填充满，有的页面一页能显示全就别滚动」。
- 新增工具 `e2e/shot-pages.js`：批量给 26 个页面截图（工具更新后截图接口可用了，
  之前多次超时）。**布局问题看图比写断言直观得多**，这轮全靠它定位。
- 修了三类问题（涉及 12 个玩法页）：
  1. **内容顶在上半屏、下方一大片空白**（24点 / 天平 / 口算冲刺 / 记忆矩阵 / 字母拼词 / 成语拼字）：
     根容器补 `min-height:100vh`（原来只有 `page{}` 有，子块 `flex:1` 吃不到空间），
     主内容块改 `flex:1 1 auto` 并在剩余空间里居中；
  2. **棋盘底槽被拉高、格子里一片空**（消消乐 / 连连看 / 2048）：棋盘改 `flex:0 0 auto` +
     `margin:auto 0`（贴着自己的格子，用 auto 外边距居中）——这是二期加底槽时引入的；
  3. **底部提示被底部指示条切掉 / 白字压浅底看不见**（2048 / 消消乐 / 连连看 / 贪吃蛇 / 数独 / 24点）：
     页面根补 `calc(20rpx + env(safe-area-inset-bottom))` 底部安全区；提示文字由白色改为深蓝灰 `#4a6b8a`。
  - 另外天平页的秤盘原先会溢出压住提示行（绝对定位没算高度），已补 `padding-bottom`。
- **顺带修一个真 bug**：连连看牌面出现「舍*忘*」「临*不*」这类带 `*` 的挖空词 ——
  `dict.filterByGroup(gradeKey, 'w1')` / `randomItemByGroup(..., 'w1')` 把**类型码当分组 key**
  （与消消乐之前修的同一个坑），改为按 `type === 'w1'` + 纯英文字母精确筛。
- 验证：`node e2e/run-all.js --no-e2e` 5/5；`--only=link` 26 条断言通过；
  26 页截图逐张过目（产物在 `e2e/reports/shots/`，gitignore）。
...
「不许出现 `prefers-color-scheme: dark`」与「app.json 不许开 darkmode / 带 themeLocation」。）

- **现象**：真机上点「排行榜、成就、我的」等页面进去**整片发黑、没有任何内容**；开发者工具里一切正常。
- **根因**：`app.wxss` 里有一段 `@media (prefers-color-scheme: dark)`，只把 `page` 底色改成近黑 `#12171f`、
  文字改浅色，但**页面内容（卡片、渐变、按钮）全是浅色主题，没有做任何深色适配**。
  统计下来有 **18 个页面自己没有背景色、依赖 `page` 底**（我的 / 成就 / 关卡 / 字母拼词 / 结算 /
  错题本 / 报告 / 昵称 / 变装 / 一笔画 …），手机一开深色模式就整片发黑。
  开发者工具默认浅色外观 → 本地完全复现不出来，所以前面几轮都没抓到。
- **修复**：移除该媒体查询（这款卡通教育游戏统一浅色主题，且多数页面本来就写死了浅色渐变背景）。
- **护栏**（`e2e/check-wxss.js`）：出现 `prefers-color-scheme: dark` 直接判红并说明原因 ——
  要支持深色模式必须逐页做完整配色 + 深色截图回归，不允许再放「只改 page 底」的半成品。
  用探针文件验证过会拦住（正常时 36 个 wxss 全过）。
- ⚠️ 复现/验证提示：真机若仍显示旧版本，可先**关掉手机的深色模式**验证同一问题是否消失，再重新上传。

### 修 bug：默认情况下怪兽立绘不显示（只剩 emoji）

- `storage.getBossSkin()` 在玩家**没主动选过皮肤**时返回空串，`getMonsterSkin('')` 走默认回退
  `DEFAULT_MONSTER` —— 而这个对象是手写的、**漏了 `image` 字段**，于是引擎不会预加载立绘，
  对局里永远只有 emoji。只有玩家去形象页选过一次怪兽皮肤，图才会出现。
- 修复：两个默认回退对象（战士/怪兽）改成**从 `SKIN_RENDER` 取字段**（单一真源，不会再漏 image）。
- 护栏：`skins.test.js` 新增用例 —— 入参为 `''`/`undefined`/`null`/未知 id 时，回退对象必须带 `image`。
- 这条是 `verify-game.js` 那句「怪兽真图已被 canvas 解码（不是 emoji 兜底）」断言抓出来的
  （之前那轮恰好存储里有值，蒙混过关了）。

### 上传成功 ✓ + 把「没人引用的工具文件」也搬出打包目录

- 用户反馈上传结果：「**代码上传成功**」✓ —— 200K 那条报错已经消除。
- 但上传提示里有 4 个文件**没有被打包上传**（`project.private.config.json` +
  `utils/parser.js` / `utils/sensitive.js` / `utils/validator.js`），原因是开发者工具开了
  「过滤未使用文件」，会把**没有任何代码引用**的文件静默跳过。
- 查证：这三个文件**确实没有被运行时引用**（只有单测在用）——它们是词库构建流程的工具
  （文本词库 → parser 解析 → validator 校验 → 生成 `miniprogram/data/*.json`），
  本来就不该待在打包目录里。按既定约定搬到 **`tools/dict/`**，引用同步更新
  （`validator.js` 继续复用小程序端的 `constants.js`，避免类型码两处定义漂移）。
  `project.private.config.json` 被排除属正常，IDE 私有配置本来就该排除。
- 新增护栏：`e2e/check-assets.js` 会**主动提示打包目录里「没有任何代码引用的 JS」**——
  开发者工具只会静默跳过、不会提醒清理。用探针文件验证过会点名报出。
- 包体：224 个文件 / 1.15MB（图片 36 张 / 0.18MB）。

### 修上传报错「图片和音频资源大小超过 200K」：判定的是总量，资源改走 URL

- **找到真正原因**：这条检测判的是**包内图片/音频的总量**，不是单个文件。代码包里
  单张最大才 60KB（一个超标文件都没有），但总量 **689KB**（成就图标 104 + 段位徽章 325 +
  立绘 256 + tab 4）早就超了 —— 这也解释了为什么反复看单张都「没超标」却一直报错。
  用户贴来的官方原文也印证了：`图片、音频等静态资源体积大小超过 200K 时，将它们上传到 CDN，
  用 URL 引入会是个更好的选择`。
- **改法（按官方建议走 URL）**：
  - 成就图标（40 张）+ 段位徽章（80 张）**移出代码包**：`miniprogram/assets/{achievements,ranks}`
    → 仓库根 **`art/`**；由云托管（同一个 Express 容器）以 `/assets/**` 静态托管
    （`server/index.js` 挂 `express.static`，`Dockerfile` 里 `COPY art/ /app/art/`，
    `.dockerignore` 特别注明 art/ 不能排除）；
  - 端上新增 `utils/art.js`：把后端返回的 `/assets/...` 拼成云托管 URL，
    用在成就页与我的页（`<image>` 加载网络图片**不需要**配服务器域名白名单）；
  - 立绘保留在包内（对局里 canvas 直接绘制，走网络有加载/失败风险），但压到 192px；
  - 结果：**包内图片 689KB → 184KB** ✓（36 张：立绘 28 + tab 8），整包 1.66MB → 1.17MB。
- **护栏**：`e2e/check-assets.js` 新增**总量硬校验**（包内图片+音频 ≤200KB，超了直接判红并提示
  「放 CDN 用 URL 引入」）+ 禁止 `assets/achievements|ranks` 再回到包里；
  新增 `tests/unit/art-url.test.js` 守 URL 转换规则与「这两类图确实不在包内」。
- 线上已验证：`/assets/ranks/bronze.png`、`/assets/achievements/answer_100.png` 均返回
  `200 image/png`；`/assets/skins/...` 返回 404（立绘本就该留在包内，符合预期）。
...
- 约定（已写进 `AGENTS.md` 第 6 节）：**`miniprogram/` 只放会被打进包的运行时代码**，
  测试 / 工具 / 美术原图 / 演示一律放打包路径以外。
- 迁移：`miniprogram/utils/__tests__/`（**37 个文件 / 241KB**）→ 仓库根 **`tests/unit/`**。
  相对路径按新旧位置精确换算（`../x` → `../../miniprogram/utils/x`、`../../x` → `../../miniprogram/x`、
  `../../../x` → `../../x`、`path.join(__dirname, ...)` 同理），迁完 37 个测试文件全部通过。
- 连带更新：`package.json` 的 `npm test`、`e2e/run-all.js` 的 unit 阶段、两个关卡生成器
  写在产物里的「校验命令」注释、以及散落在源码/文档里的路径引用。
- **清空 `packOptions.ignore`**（原来靠 ignore 把 `utils/__tests__`、`assets-src` 排除在包外 ——
  「眼不见为净」不算数，现在打包目录里压根没有这些东西）。
- 新增**打包目录纯净度护栏**（`e2e/check-assets.js`）：出现 `__tests__` / `*.test.js` /
  `assets-src` / `e2e` / `demo` 直接判红退出码 1。造了个 `miniprogram/utils/__probe.test.js`
  探针验证过：确实报错并点名文件，探针已删。

### 真机登录失败无法排查：把真实错误弹出来（用户反馈「真机上排行榜等都进不去」）

- 现象：真机上排行榜 / 每日一题 / 关卡解锁 / 我的页同步 这些入口都进不去。
  代码层面确认：这些入口都由 `auth.isLoggedIn()` 把关，未登录会走 `auth.promptLogin()`。
- 根因线索：`promptLogin` 登录失败时**只弹一句「登录失败，请检查网络后重试」**，
  而底层 `request.js` 其实已经拿到了真实原因（`errMsg` + `errCode`）—— 被丢掉了，线上只能靠猜。
  （首页那條 `_doLogin` 路径本来就会弹真实错误，所以只有「从入口页触发的登录」看不到原因。）
- 修复：`promptLogin` 失败时改为弹 **modal**，内容 = 原始错误 + 两条最常见原因的对照：
  ① 提示 env / 环境相关 → 云开发环境没关联到本小程序；
  ② 提示 -501000 / 服务不存在 → 云托管服务名或环境 ID 不对。
  完整错误对象同时打到 console（真机可用 vConsole/远程调试看）。
- 背景（为什么真机可能是云连接问题）：`utils/request.js` 走的是 `wx.cloud.callContainer`
  （envId `prod-d6gnifjoe28cfd96f` + 服务名 `express-g0hk`）。这条链路要求
  **该环境已关联到本小程序 appid**；关联缺失时开发者工具能用、真机不行，与本次现象一致。

### 2048 去掉方向键按钮，滑动成为唯一操作（用户反馈「上下左右按钮没啥用」）

- 用户判断正确，且有实测依据：滑动手势本来就实现了（`onTouchStart/onTouchEnd`，
  24px 阈值 + 方向判定），而三排方向键白占屏幕约 **22% 的高度**（3×88rpx + 间距 ≈ 300rpx ≈ 150px）。
- 改动：删掉 `.pad`（wxml + wxss + `onDir` 方法，全仓库已确认无残留引用），
  底部提示改成「👆 朝上下左右滑动棋盘 · 相同数字合体」；选关页原本就写着「滑动合成目标数字」，
  引导没有缺失。
- **顺手加强滑动手感**：棋盘加 `catchtouchmove`（空实现），阻止页面跟着手指滚 ——
  否则竖滑会被页面滚动吃掉一部分，玩家会觉得「滑了没反应」。
- 端到端不受影响：`verify-g2048.js` 一直是通过 `_move` 驱动的（不依赖按钮），
  留档说明「滑动、计步、失败判定都是真实逻辑」。
- 预览工具同步：`e2e/preview-boards.js` 的 2048 画框去掉方向键，改画底部提示。

### 单词闯关对局界面优化（HUD + 选项按钮）

- **选项按钮**改成「上浅下深」的两段渐变 + 顶部内高光 + 文字硬阴影（糖果按钮），
  与二期棋盘的材质语言统一；原来 5 种配色都是纯色块，看着平。
- **HUD 重排**：`.hud-chip` 高度 76 → 72rpx、内边距 18 → 14rpx、间距 8 → 6rpx，
  得分/题号字号各降 2rpx，图标按钮 72 → 60rpx；胶囊加了一层顶部内高光。
- ⚠️ 顺带修掉一个**真实布局缺陷**：终关 Boss 是 **7 颗心**，而 HUD 是按 5 颗心调的 ——
  实测（`e2e/preview-game.js` 的 Boss 场景，7 命 + 得分 110 + 第 12/15 题）
  命数胶囊会被挤成两行、五个胶囊里有一个变高。现在给命数加了
  `flex: 0 0 auto` + `white-space: nowrap`（不许压缩、不许换行），并收紧其余胶囊，
  四种场景实测都是单行、最宽 319/351px，留有余量。
- 新增 `e2e/preview-game.js`：整屏预览（HUD + 画布 + 提示 + 选项），
  样式取自**真实的 app.wxss / pages/game/game.wxss**，画布几何取自渲染层纯函数，
  含「Boss 7 命」这类边界场景 —— 这次就是靠它发现 7 颗心换行的。

### 成就图标补齐 40/40（用户到货最后 2 张）+ 包体回正

- 用户补上最后两张成就图标原图：`custom_level_1`（出题人，羽毛笔）与
  `custom_level_5`（题库作者，关卡卡册+星星），512×512 原图放在 `assets-src/achievements/`。
- 跑一遍 `e2e/compress-assets.py` 后，**包内成就图标 38 → 40 张、缺图 0**，
  成就页 40 条从此**全部有图**、不再有 emoji 混排。
- ⚠️ 顺带发现并修掉一次包体超限：包内那 38 张成就图标被换成了 **256×256 的未量化版本**
  （单张 32~60KB），整包从 1.64MB 涨到 **2.65MB**，直接冲过 1.8MB 上限（静态层当场判红）。
  重新压缩后：图标回到 128px/96 色（单张几 KB），整包 **1.66MB** ✓。
  原图始终保留在 `assets-src/`，要更高画质随时可以把 `compress-assets.py` 里的
  尺寸/色数调高再跑（当前还有 ~0.14MB 余量）。

### 对局画面改版：怪兽当主体，题目改成「挂名牌」（用户反馈「单词挡住怪兽」）

- 问题：上一版把怪兽真图**画在 250×118 题目卡片后面**，卡片几乎盖住了整只怪兽，
  只在卡片上沿露出个脑袋尖 —— 用户原话「你这怪兽和单词词语没有很好的融合起来呢，现在单词挡住怪兽了」。
- 改法：**怪兽升为画面主体，题目降级为挂在它肚子上的小名牌**。
  - 怪兽图高度 140 → **158**、底边对齐卡片底边（头自然探到卡片上方）；
    开局卡片贴顶时把整图下移（`topMin: 0`），被切掉的是下缘 —— 那块本来就被名牌挡着；
  - 新增 `monsterPlateLayout(textW, mx, my)`：名牌**宽度跟着题目文字走**
    （`plateMinW 132` / `plateMaxW 186` / 左右各留 16），保证怪兽的脑袋、角、脸颊、手臂在牌子四周都露得出来；
  - 名牌用**肤色 88% 不透明度**填充，兽肚能透一点出来，避免像「另贴上去的一张卡」；
    答题反馈（答错红框 / 震屏 / 暴怒放大）照旧作用在名牌上。
- 护栏：单测扩到 **14 用例 / 242 断言**，新增三条名牌用例，其中一条是**观感红线** ——
  `plateMaxW` 必须小于「最窄那只怪兽」的宽度，否则又会退化成「名牌把怪兽盖住」。
  几何口径变了，旧的三条怪兽用例同步重写（改成「高度固定、宽度按原图比例」）。
- 验证方式：新增 `e2e/preview-monster.js` —— 直接把渲染层导出的纯函数算出的几何喂给浏览器渲染，
  出 16 张对局画面（4 只怪兽 × 4 种状态）截图肉眼比对；因为用的是同一套函数，预览不会与真机漂移。

### 需求⑥ 第二期：棋盘 / 卡牌材质升级（6 个棋盘类玩法）

- 一期把头部与 HUD 统一了，这期统一**棋盘本身**。原来 6 个页面（数独 / 2048 / 24 点 /
  连连看 / 消消乐 / 贪吃蛇）的格子都是一块纯色方块 + 几乎看不见的阴影，整块棋盘像浮在页面背景上。
- 在 `app.wxss` 定一套共用材质（不再各页各写一份），页面里只加类名：
  - `.b-tray` 棋盘底槽（让棋子「嵌」在板上）→ 6 个棋盘全部套上，另加一次性入场动画 `.b-in`；
  - `.b-face` 棋子/卡牌面（白面渐变 + 底部硬阴影 + 顶部内高光，卡通塑料片质感）；
  - `.b-press` 按下反馈（下沉并收起投影，与既有 `.btn:active` 同一套手感）；
  - `.b-sel` 选中态（金色环 + 上浮）。
- **顺手修掉两个「写了等于没写」的样式**：`math24` 的 `.card.sel` / `.card.merged` 都只设了
  `border-color` 却从没声明过 `border`，真机上只有一层很淡的底色，玩家看不出哪张牌被选中。
  现在改用 `box-shadow` 画环（不依赖 border），选中=金环、刚合并=绿环。
- 状态色一并收紧：数独「题面给定的格子」改成凹面灰蓝（与玩家填的区分）、填错/错配/未命中改粉色渐变、
  消消乐命中与连连看消掉改成「缩小淡出」（原来是直接消失）、贪吃蛇的蛇头/蛇身/食物改成渐变并加投影。
- 波形：只改材质与状态色，**不动任何布局尺寸与交互逻辑**；
  2048 的彩色方块靠 `.cell.cN`（两类的选择器）压过 `.b-face` 的单类背景，所以数字配色不受影响。
- **验证方式**：新增 `e2e/preview-boards.js` —— 把真实 wxss 转成浏览器 CSS 渲染 6 个手机画框，
  用浏览器截图肉眼确认（本机模拟器截图接口是挂的）。踩坑记录：预览必须给页面样式加
  `.pg-<页面>` 作用域前缀，否则 `.cell`/`.tile` 这些同名类会跨页面互相覆盖，
  第一版就因此误判「选中态没生效」。工具用法写进了 `e2e/README.md`。

### 需求⑤ 遗留 A：怪兽美术到货并接入对局（4 套）

- 素材：豆包出的一张 4 只纵向拼图（1024×4096），但导出的是**白底 RGB、没有透明通道** ——
  直接进包会在游戏里显示一块白板。新增 `e2e/prepare-monsters.py` 一次做四件事：
  **等分切图 → 从四边泛洪抠白底 → 裁到「头+上半身」→ 缩到 340px + 调色板 PNG 压缩**。
  抠图只删「与画布边缘连通的白色」，所以怪物内部的眼白/雪花/冰晶都完好；边缘再做 0.6px alpha 羽化去锯齿。
  实测单张 **12.7~16.6KB**（合计 59.1KB），远低于 200KB 红线；整包 1.58MB → **1.64MB**。
- 对局呈现（`game/renderer.js`）：真图画在**题目卡片之前**，被卡片挡住下半身，像怪兽「举着题目卡片」；
  卡片上的「角 + 眼睛 + emoji」是没有真图时的兜底造型，有真图时**跳过**（否则会变成四只眼）。
  - 抽成纯函数 `monsterArtLayout(iw, ih, mx, my)` + 常量 `MON_ART`，**只位移不缩放**
    （第一版按可用空间压缩高度 → 开局会把角色压扁，是靠浏览器预览图发现的）；
  - 关键约束：开局卡片上方只剩 22px，所以图片顶边夹到 2px、整图下移（下半身本来就被卡片挡），
    下沉约 2 秒后头肩完整露出（约 70px）；裁剪边界用未变换坐标系的卡片底边，缩放/震屏都不会露出腿。
- 尺寸取 170 宽的原因（踩过）：图越大越会被顶部夹住、**露出来的反而越少**；170 是「露得多 + 不顶出画布」的平衡点。
- 顺手修掉一个真 bug：`getMonsterSkin()` 组装返回值时**漏了 image 字段**（`SKIN_RENDER` 里加了、函数没返回），
  而引擎正是拿 `G.monsterSkin.image` 去预加载真图的 → 结果对局里永远退回 emoji。
- 护栏：`skins.test.js` 补三条契约 —— 怪兽 4 套必须有图且文件真实存在、`LOCAL_SKINS` 与 `SKIN_RENDER`
  **图片路径必须一致**（这次就是只改了一张表）、`getMonsterSkin/getWarriorSkin` 必须带出 image；
  `renderer-warrior.test.js` 增至 12 用例/216 断言；端到端新增 `_testSkinImage()` 钩子，
  `verify-game.js` 断言 **canvas 真的解码出怪兽/战士真图**（39 条断言）—— 这条正是抓出上面那个字段缺失的用例。
- 下次让豆包出图**务必透明底导出**；踩坑已回写 `docs/美术素材需求与豆包提示词.md`。
