...
- 新增 `utils/challenge-rewards.js`：里程碑 = 通 10/20/30 关各一个宝箱，奖励 **+10/+20/+30 星**
  （星星是段位货币，直接补上「爬到段位中间级没收益」的体感；学段满通共 +60 星）。
  - 领取**幂等**（已领再点只提示，不重复加星），记录存本地 `ww_chest_claimed`（键 = 学段:关数），按学段隔离；
  - 「已通关」口径 = 该学段挑战主线里 ≥1 星的关卡数（0 星不算，自由练存档不计入）；
  - 登录玩家领取时同步段位（`/api/rank/sync`）。
- 关卡页「挑战主线」视图新增三个宝箱卡（锁 / 可领取高亮 / 已领取三态），点可领取的弹「+N 星」。
- 单测 `challenge-rewards.test.js`（7 用例/36 断言）+ 端到端 `verify-challenge.js` 增至 **73 条断言**
  （含「只通 10 关时 20 关宝箱仍锁着」「重复领取幂等」）。

> 已知取舍（记在交接单）：游客领取只记本地、之后登录不补记段位；要补发需把宝箱记录搬到服务端，当前不做。

### 修 bug：`unlockType='level'` 的皮肤永远解锁不了

- 上架 24 套皮肤时给「学者之王」用了 `unlockType='level'`，但**服务端解锁接口只实现了 free/stars/rank**
  → 落到「条件未满足」，这套皮肤线上永远是锁的。
- 修复：服务端补 `level` 分支（口径 = `RankRecord.wins` 累计通关次数 ≥ unlockValue，即「累计通关 30 次」）；
  头像页解锁文案同步改成「累计通关 N 次解锁（当前 M 次）」。
- 护栏：新增 `avatar-unlock.test.js`（3 用例/14 断言）——静态契约校验
  ① 服务端为每个可手动解锁类型都有判定分支；② 前端文案覆盖同一套类型；③ 皮肤目录里出现的 unlockType 必须都被支持
  （否则就是「解锁不了的死皮肤」）。

### 挑战主线终关 Boss（方案 A：题量 ×1.5 + 命数同比例放大）

- 每个学段**第 30 关**固定为终关 Boss：玩法用最有代表性的**字母射击**（与第 1 关「新手关」首尾呼应），
  参数 **15 题 + 7 命**（普通关 10 题 + 5 命），星级阈值不动。
- 为什么必须同时放大命数（这是当初把 Boss 推迟的原因，本次先用 `starReachability` 算清才动手）：
  通关要求答对 `总题数 −（命数 − 1）` 题 ——
  `10 题 5 命 → 最低 60%`（恰好压在 1 星线）、`15 题 5 命 → 最低 73%`（1★ 不可达）、
  `15 题 7 命 → 最低 60%`（三档重新可达）。护栏把**不可行的老方案**也钉住了：
  `starReachability(15,5)` 必须是 `[2,3]`、`starReachability(10,4)` 也是 `[2,3]`，改回去就红。
- 引擎层新增「题量/命数可由关卡注入」：`engine.start(..., { totalQ, lives })`，不传则维持 10 题 5 命，
  老调用方零影响；`reset()` 也按本局命数复位（否则「再玩一次」会悄悄退回 5 命）。
- 六个玩法页统一把**关卡号**传进 `challenge.paramsOf(grade, mode, level)`（Boss 参数只在这条路径上生效）。
- UI：关卡页 Boss 行加金色描边 + `BOSS` 徽标（未解锁时编号方块仍是灰色，不误导），
  首页「继续挑战」卡片在指向 Boss 时前缀 `👑 BOSS`。
- 测试：`challenge.test.js` 22 用例/461 断言（新增 Boss 参数、星级可达性、题库真能取到 15 题三组）；
  端到端 `verify-challenge.js` 扩到 **86 条断言**，并**走真实用户路径**验证 ——
  前 29 关点满星 + 模拟登录 → 首页继续挑战指向第 30 关（文案 `👑 BOSS · … · 第 30/30 关`）→
  点进 Boss 对局 → 实测 `totalQ=15`、HUD 7 颗心、`第 1/15 题`、4 个选项正常出题。
- ⚠️ 端到端编排踩坑记录：**一条用例只允许加载一次对局页，且必须是最后一步**。
  先跑第 1 关对局再 reLaunch 到 Boss 对局会稳定 timeout（开发者工具的 canvas rAF 老问题），
  所以改成「前 29 关满星 → 继续挑战直接进 Boss」的单一对局页路径。
...
- `e2e/check-assets.js` 补齐音频扫描（`.mp3/.wav/.m4a/.aac/.ogg/.wma/.flac/.amr`），
  把「单文件 ≤ 200KB」定为**图片 + 音频共同的硬限**（原来只守图片、且阈值是项目自定的 60KB）；
  60KB 降级为**只提醒不阻断**的建议值 —— 它影响的是整包余量，不该和平台硬限混为一谈。
- 新增护栏 `miniprogram/utils/__tests__/assets-limit.test.js`（4 用例）：静态读校验脚本，
  断言 ① 硬限恒为 200KB、只能更严不能放宽；② 音频扩展名在扫描范围内；
  ③ 整包阈值不高于 1.8MB；④ 真扫一遍 `miniprogram/` 确认当前素材全都没超标。
- 实测：现包 **150 张图片 / 最大 9.3KB / 合计 0.61MB，音频 0 个**（音效是 `game/audio.js` 用 WebAudio
  现场合成的，发音走 TTS 插件，包内本就没有音频资源）；
  并造了一个 250KB 的假音频探测文件验证拦截确实生效（退出码 1，验完立即删除）。
- 规则同时写进项目级 `AGENTS.md`（新增第 6 节），新增素材时按它执行。

### 需求⑤ 遗留 B：战士开火反馈（对局内皮肤动态展示）

- `game/renderer.js` 新增纯函数 `warriorRecoil(state)`：按**炮弹离炮口的距离**算后坐力与炮口闪光强度，
  `warriorLayout(now, recoilDy)` 支持「只压战士本体、底座不动」；闪光颜色取当前皮肤主色
  （换皮肤 = 换闪光色，延续「越高级越好看」）。
- **为什么不用时间戳驱动**：小程序 canvas 的 rAF 时间戳在模拟器/后台会节流甚至停摆
  （项目里踩过「选项点对了炮弹永远飞不到」）；按距离算与游戏状态严格同步，`_testStep` 手动步进也能复现。
- 参数取 140px 作用范围（炮弹 700px/s ≈ 200ms 观感）—— 52px≈74ms 人眼基本看不到。
- 护栏：`renderer-warrior.test.js` 扩到 8 用例/171 断言（空状态不动、刚出膛最强、单调衰减且有界、
  方向不影响强度、只压本体、头顶不压出画布）；端到端 `verify-game.js` 增至 36 条断言
  （真实点击 → 立刻读强度 dy=8/t=1 → 步进到飞远 → 归零），新增测试钩子 `pages/game._testRecoil()`。

### 终关 Boss：原方案被证明不可行，改为待拍板

- 复盘 `challenge.starReachability` 口径后确认：**5 命 + 1★ 阈值 60% 时，题量一旦超过 10 就必然出现 1★ 死区**
  （通关必须答对 `total-(lives-1)` 题：10 题→60% 恰好可达，15 题→73%、10 题 4 命→67% 都不可达）。
  即用户原话里的「加题量/减命数」两种做法都会重演 R1 修掉的老缺陷，**故本轮不动手**，
  已把证据、三个候选方案（A 题量+命数同比例放大 / B 限时 / C 取更难的词）写进交接单等用户拍板。

### 验证流程固化：整轮 E2E 开跑前自动重启开发者工具

- 起因：上一轮 E2E 被中断（Ctrl+C / 超时被杀）后，开发者工具会停在中断时那个页面；
  下一次 `cli auto` 只是**复用**已开着的工具、不会重开项目，于是整轮里第一个阶段读到遗留页面，
  报出「实际路径 = pages/link/link」这类**假失败**，还会把后面几个阶段一起带崩
  （本轮真实踩到：15 阶段里 5 个失败，逐个单跑全部通过）。
- 修法：工具路径 / cli 调用收敛到 `e2e/lib/devtools.js`（harness 与编排器共用同一份定义）；
  `run-all.js` 在本轮首个 E2E 阶段开跑前 `quit` 一次并留 5s 冷却，强迫下一个 `cli auto`
  拉起干净的工具与最新编译；`ensureAutomation` 的 `cli auto` 增加一次自动重试
  （工具刚退出的瞬间 auto 会静默失败）。
- 开关：`node e2e/run-all.js --no-reset` 可跳过重启；换机器用 `WX_DEVTOOLS_DIR` 指定安装路径。

### 本轮验证

- `node e2e/run-all.js` 从干净工具起跑 **15/15 全绿**（542.3s），报告见 `e2e/reports/latest.json`。

---

## 2026-09-12（挑战主线 P2：6 款玩法轮换 + 各玩法成绩上报 + 玩法类成就启用）

### 主线编排：3 款 → 6 款

- `utils/challenge.js`：玩法轮换改为 **字母射击 → 消消乐 → 字母拼词 → 连连看 → 成语拼字 → 单词贪吃蛇**，
  每学段 30 关 = 6 款 × 5 关；每款补上 `gameType`（成绩表 `game_type`，小写 snake_case——
  服务端只接受 `[a-z0-9_]`，`wordBuild` 这种驼峰会落回默认值）和按学段的题量/对数/词数参数。
- **老存档迁移规则随之修正**：旧「字母射击 10 关」按**序号**平移到主线第 1~10 关
  （原来按「第 k 个字母射击关」映射，P2 后字母射击只剩 5 关，旧第 6~10 关会无处安放——单测当场抓到）。

### 三款新玩法接入（都支持「按关卡参数开局 + 固定种子」）

- **词义消消乐** `pages/match`：学段/对数按关卡参数，取词与洗牌走关卡种子（同关同牌面），
  结算写挑战星级并上报成绩。**顺带修掉一个真 bug**：取词原来写的是 `filterByGroup(key, 'w1')`——
  `w1` 是类型码不是分组 key，会静默放行整库，词卡可能出现 `愚*移*` 这种带 `*` 的挖空词
  （与贪吃蛇修过的是同一个坑），现在按 type 精确筛 w1 + 纯字母兜底，端到端加了断言。
- **成语拼字** `pages/idiom-build`：与字母拼词共用引擎，接同样的挑战参数（题量按学段、种子固定题面与字块）。
- **单词贪吃蛇** `pages/snake`：挑战模式学段由关卡固定、词表按种子决定；结算写挑战星级并从统一入口上报成绩。

### 各玩法成绩上报（新增 `utils/play-report.js`）

- 原来只有字母射击（结算页）会上报成绩 → 「玩法进度榜」长期只有射击数据、玩法类成就无法判定。
  现在 **字母拼词 / 连连看 / 消消乐 / 成语 / 贪吃蛇 / 数独 / 24 点 / 记忆矩阵** 都会上报
  `{ game_type, grade, level, score, correct, total, stars }`（登录才报、失败静默、拿星时同步段位）。
- 顺带启用美术交付但一直闲置的 4 张玩法成就图标：**贪吃蛇大师 / 24 点高手 / 数独行家 / 记忆大师**
  （指标 = 按 `game_type` 统计的通关次数，阈值 3 次）；成就总数 36 → **40 条**。

### 测试

- `challenge.test.js`：编排改为 6 款各 5 关、迁移规则改为按序号平移（19 用例）。
- `verify-challenge.js` 扩到 **66 条断言**：新增三款玩法的「同关两次进入一致 + 与 Node 侧独立复算一致」
  与「消消乐词卡必须纯英文」；关卡行副标题、第 2~6 关玩法标签同步更新。
- `achievements.test.js`：新增「按 game_type 统计各玩法通关次数」用例，并修掉真实查询里
  `loadStats` 漏传 `game_type`（否则这 4 条成就在线上永远解不开——护栏当场拦住）。
